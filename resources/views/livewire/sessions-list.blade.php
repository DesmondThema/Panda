<div>
    ...
    <div class="mt-4">
        @if (empty($sessions))
            <p class="text-gray-500">No sessions match the selected filters. You can either clear the filters or try different ones.</p>
            <button wire:click="clearFilters" class=" text-blue-500 underline">Clear All Filters</button>
            <iframe src="https://giphy.com/embed/5rQJdo97swSTBQwhRA" width="240" height="240" frameBorder="0"></iframe>
        @else
            <ul role="list" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                @foreach($sessions as $session) <li class="bg-white px-4 py-6 shadow sm:rounded-lg sm:p-6">
